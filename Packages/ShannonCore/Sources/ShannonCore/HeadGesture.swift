...
/// One attitude sample from a head-tracked device (AirPods Pro / Max), in radians.
///
...
public struct HeadGestureConfig: Sendable, Equatable {
    ...
    /// Shared by the Mac pill and the iPhone app, so a nod that confirms on
    /// one device feels identical on the other.
...
///  * **Disarmed by default.** `arm()` is called only while a confirmation is
///    actually pending. Samples fed while disarmed are dropped, so ordinary
///    head movement can never fire a confirmation.
...
public protocol HeadphoneMotionProviding: AnyObject, Sendable {
    ...
    func start(onSample: @escaping @Sendable (HeadAttitudeSample) -> Void)
    func stop()
}

/// Deterministic source for tests and previews.
public final class StubHeadphoneMotionProvider: HeadphoneMotionProviding, @unchecked Sendable {
    private let lock = NSLock()
    private var handler: (@Sendable (HeadAttitudeSample) -> Void)?
    ...
    public func start(onSample: @escaping @Sendable (HeadAttitudeSample) -> Void) {
    ...
    public func emit(_ sample: HeadAttitudeSample) {
        lock.lock()
        let h = handler
        lock.unlock()
        h?(sample)
