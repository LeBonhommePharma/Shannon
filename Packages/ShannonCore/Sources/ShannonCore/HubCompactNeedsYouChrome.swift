...
public enum HubCompactNeedsYouChrome: Sendable {
    ...
    ///
    /// - Parameters:
    ///   - isCompact: True when `HubLayout` resolves to `.compact`.
    ///   - hasPendingAsk: Any non-expired pending confirmation.
    ...
    /// Agents that belong in the pinned needs-you band (blocked **or** open
    /// confirmation). Preserves caller order (typically already ranked).
    ...
    ///
    /// When `shouldPin` is false, returns `([], agents)` so callers can keep a
    /// single loop path without reordering.
