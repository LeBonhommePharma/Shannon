...
final class HubCompactNeedsYouChromeTests: XCTestCase {
    ...
    func testShouldPinOnlyWhenCompactAndPendingAsk() {
        XCTAssertTrue(
            HubCompactNeedsYouChrome.shouldPin(isCompact: true, hasPendingAsk: true)
        )
        XCTAssertFalse(
            HubCompactNeedsYouChrome.shouldPin(isCompact: true, hasPendingAsk: false)
        )
        XCTAssertFalse(
            HubCompactNeedsYouChrome.shouldPin(isCompact: false, hasPendingAsk: true)
        )
        XCTAssertFalse(
            HubCompactNeedsYouChrome.shouldPin(isCompact: false, hasPendingAsk: false)
        )
    }
    ...
    func testShouldPinFiltersExpiredConfirmations() {
        ...
        XCTAssertTrue(
            HubCompactNeedsYouChrome.shouldPin(
                isCompact: true,
    ...
    func testShouldPinFromSnapshot() {
        ...
        XCTAssertTrue(
            HubCompactNeedsYouChrome.shouldPin(isCompact: true, snapshot: snap, now: now)
        )
        XCTAssertFalse(
            HubCompactNeedsYouChrome.shouldPin(isCompact: false, snapshot: snap, now: now)
        )
        XCTAssertFalse(
            HubCompactNeedsYouChrome.shouldPin(
                isCompact: true,
    ...
    func testNeedsYouAgentsIncludesBlockedAndPending() {
        ...
        let needs = HubCompactNeedsYouChrome.needsYouAgents(
            from: agents,
            pendingAgentIDs: ["ask"]
        )
    ...
    func testPadHubWiresCompactNeedsYouPin() {
        ...
        XCTAssertTrue(
            grid.contains("pinNeedsYou") || grid.contains("HubCompactNeedsYouChrome"),
            "DashboardGridView must accept / apply pinNeedsYou for compact elevate"
        )
        XCTAssertTrue(
            grid.contains("partitionForDisplay") || grid.contains("needsYou"),
