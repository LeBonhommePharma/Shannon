...
final class PetCodexMotionTests: XCTestCase {
    ...
    private static func signals(
        ...
        // Use raw-value table only for statuses both sides share in the golden.
...
final class PetPackageResolverTests: XCTestCase {
    ...
    // MARK: - B3 agent → package id

    func testPreferredPackageIdMapsKnownAgents() {
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "grok_build"),
            "grok"
        )
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "Grok-Build"),
            "grok",
            "hyphen/case normalize"
        )
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "science"),
            "shannon"
        )
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "cursor"),
            "bonhomme-cat"
        )
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "design"),
            "firebear"
        )
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "cowork"),
            "bonhomme"
        )
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "dispatch"),
            "firebear"
        )
    }

    func testPreferredPackageIdStylePetFallback() {
        let ravenStyle = AgentStyle(
            id: "custom_raven",
            displayName: "Custom",
            shortName: "CR",
            systemImage: "bird",
        ...
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "custom_raven", style: ravenStyle),
            "grok"
        )
        let catStyle = AgentStyle(
            id: "custom_cat",
        ...
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "custom_cat", style: catStyle),
            "bonhomme-cat"
        )
    }
    ...
    func testPreferredPackageIdPreferenceOverrideWins() {
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(
                forAgentId: "grok_build",
                preferenceOverride: "firebear"
            ),
            "firebear"
        )
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(
                forAgentId: "science",
                preferenceOverride: "  collapse-cat  "
            ),
            "collapse-cat"
        )
        // Empty / whitespace override ignored → agent map.
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(
                forAgentId: "grok_build",
                preferenceOverride: "   "
    ...
    func testPreferredPackageIdUnknownFallsBackToDefault() {
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "totally_unknown_xyz"),
            PetPackageResolver.defaultPetId
        )
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: ""),
            PetPackageResolver.defaultPetId
        )
        // Package slug as agent id is accepted.
        XCTAssertEqual(
            PetPackageResolver.preferredPackageId(forAgentId: "firebear"),
            "firebear"
        )
    }
    ...
    func testPreferredPackageIdCatalogAgentsNeverBlank() {
        for style in AgentStyleCatalog.all {
            let pid = PetPackageResolver.preferredPackageId(forAgentId: style.id)
            XCTAssertFalse(pid.isEmpty, style.id)
            XCTAssertFalse(pid.contains(" "), "package id must be slug: \(pid)")
        }
    }
