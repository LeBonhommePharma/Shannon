...
class CredentialManager:
    ...
    @classmethod
    def rotate_if_needed(cls, agent_id: str, expires_at: Optional[float], margin_s: float = 300.0) -> None:
        """
        Rotate (re-fetch/refresh) a cloud agent's token if `expires_at`
        (epoch seconds) is within `margin_s` (default 5 min) of now.

        This is a hook point: the actual refresh call is agent-specific and
        left to the concrete cloud integration, but every outgoing request
        must consult this before firing so tokens don't expire mid-flight.
        """
        if AGENT_AUTH_TYPE.get(agent_id) != "cloud":
            return
        if expires_at is None:
            return
        if expires_at - time.time() <= margin_s:
            warnings.warn(
                f"[{agent_id}] token expires in <{margin_s:.0f}s — rotation required "
                f"before further requests",
                stacklevel=2,
            )
            raise AuthError(
                agent_id,
                f"token expires within {margin_s:.0f}s window; refresh required "
                f"before send",
            )


def require_credentials(func: Callable) -> Callable:
    """
    Decorator: enforce CredentialManager.credential_check() for the calling
    AgentClient's agent_id before any outgoing request executes.

    Applied to every method that puts a message on the wire (_send / _send_http
    / _send_socket entry point) so no cloud agent can transmit with a missing
    or rejected token.
    """
    import functools

    @functools.wraps(func)
    def wrapper(self: "AgentClient", *args: Any, **kwargs: Any) -> Any:
        CredentialManager.credential_check(self.agent_id)
        expires_at = getattr(self, "_token_expires_at", None)
        CredentialManager.rotate_if_needed(self.agent_id, expires_at)
        _RATE_LIMITER.check(self.agent_id)
...
class RateLimitExceeded(Exception):
    """Raised when an agent exceeds its request quota. Carries retry_after (seconds)."""

    def __init__(self, agent_id: str, retry_after: float) -> None:
        super().__init__(
            f"[{agent_id}] rate limit exceeded — retry after {retry_after:.1f}s "
            f"(HTTP 429)"
        )
        self.agent_id = agent_id
        self.retry_after = retry_after
        self.status_code = 429
        self.headers = {"Retry-After": str(int(retry_after) + 1)}


class RateLimiter:
    """
    Simple in-process sliding-window rate limiter: N requests per 60s window,
    per agent_id. Thread-safe.
    """
    ...
    def check(self, agent_id: str) -> None:
        """Raise RateLimitExceeded if agent_id has exceeded its quota."""
        now = time.time()
        ...
        with self._lock:
            ...
            if len(events) >= self.max_per_minute:
                ...
                raise RateLimitExceeded(agent_id, retry_after)
            events.append(now)
